...
`/Users/jaydenkang/Desktop/New Projects/20260501_5Color Harness/evals/trigger_eval.json` 에 20개 쿼리 작성. should-trigger 10개 + should-not-trigger 10개.
...
run_loop.py는 `claude -p` 서브프로세스를 호출하므로 현재 세션 모델 ID를 그대로 넘긴다. 이 세션은 `claude-opus-4-8`.
...
```bash
cd "/Users/jaydenkang/Desktop/New Projects/20260501_5Color Harness"
...
`/Users/jaydenkang/Desktop/New Projects/20260501_5Color Harness/evals/output_quality_eval.md` 에 다음 6개 케이스를 만들어 각각 3회 호출.

| ID | 입력 | 검증 항목 |
|----|------|-----------|
| Q1 | "시장 조사 보고서" | 9.5군. 분석·보고서 카테고리. 결론 메모 톤(단정형+행동 동사+주체). 추측 어미 0회. 숫자 출처 강제. |
| Q2 | "이메일" | 9.5군. 카테고리 라우팅 정확. 분량 1500~2800자. |
| Q3 | "사과문" | 9.7군. 직접 인용 금지 2개 이상. BLACK 제약 2개. |
| Q4 | "블로그" | 9.0군. 특화 금지 1~2개로 절제. |
| Q5 | "영업 덱이랑 발표 스크립트 같이" | 복합 산출물. 본체+부속 흡수. GOLD 단일 독자. |
| Q6 | "make a system prompt for board memo" | 영문 트리거. english-augment 적용. GOLD 영문 환경. |
...
| 출력 체크리스트 통과율 | **10개 항목 모두 통과 × 6개 케이스 × 3회 = 180/180** | 한 항목이라도 한 번이라도 빠지면 fail |
...
python /var/folders/2_/9tvc_jzd4vs_x2p7vd_3ffxh0000gn/T/claude-hostloop-plugins/2d7a646e17e17c3e/skills/skill-creator/eval-viewer/generate_review.py \
  5color-workspace/iteration-1 \
  --skill-name "5color" \
  --benchmark 5color-workspace/iteration-1/benchmark.json \
...
python -m scripts.run_loop \
  ...
  --model claude-opus-4-8 \
  --max-iterations 5 \
  --previous-workspace 5color-workspace/iteration-1 \
  --verbose
```
